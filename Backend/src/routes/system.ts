...
import { systemLogs, logSystemEvent } from '../data/mockData';
...
router.get('/status', (req, res) => {
  ...
  const generateMetrics = (baseCpu: number, baseRam: number) => {
    return {
  ...
  if (Math.random() > 0.6) {
    ...
    const levels: ('info' | 'warn' | 'error')[] = ['info', 'info', 'info', 'info', 'warn'];
    const idx = Math.floor(Math.random() * messages.length);
    const serviceIdx = Math.floor(Math.random() * services.length);
    logSystemEvent(services[serviceIdx], messages[idx], levels[idx]);
  }
