...
router.post('/login', async (req, res) => {
    try {
        ...
        const isValid = await bcryptjs_1.default.compare(password, employee.passwordHash);
        if (!isValid) {
        ...
        try {
            ...
            logger_1.logger.warn(`Failed to parse permissions for ${employee.empCode}`);
        }
        const payload = {
            id: employee.id,
        ...
        logger_1.logger.error(`[Auth] Login error: ${error.message}`);
...
router.post('/register', authenticate_1.authenticate, (0, authorize_1.requireAdmin)(), async (req, res) => {
    try {
        ...
        logger_1.logger.error(`[Auth] Registration error: ${error.message}`);
        res.status(500).json({ error: 'Failed to register employee' });
    }
});
// PUT /api/auth/change-password
router.put('/change-password', authenticate_1.authenticate, async (req, res) => {
    try {
        ...
        const isValid = await bcryptjs_1.default.compare(oldPassword, employee.passwordHash);
        if (!isValid)
        ...
        logger_1.logger.error(`[Auth] Change password error: ${error.message}`);
