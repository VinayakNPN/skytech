...
exports.STANDARD_WBS_TASKS = exports.DEFAULT_WBS_PHASES = void 0;
exports.ensurePhasesExist = ensurePhasesExist;
exports.seedStandardWBSTasksForInquiry = seedStandardWBSTasksForInquiry;
exports.markPhase1CompletedForInquiry = markPhase1CompletedForInquiry;
const prisma_1 = require("../db/prisma");
exports.DEFAULT_WBS_PHASES = [
...
async function ensurePhasesExist() {
    const phaseMap = {};
    for (const p of exports.DEFAULT_WBS_PHASES) {
        const existing = await prisma_1.prisma.wBSPhase.upsert({
...
async function seedStandardWBSTasksForInquiry(inquiryId, isConfirmed = false) {
    const phaseMap = await ensurePhasesExist();
    const existingCount = await prisma_1.prisma.wBSTask.count({
        where: { inquiryId }
    });
    if (existingCount >= exports.STANDARD_WBS_TASKS.length) {
        return;
    }
    for (const t of exports.STANDARD_WBS_TASKS) {
        ...
        const existingTask = await prisma_1.prisma.wBSTask.findFirst({
 ...
 */
async function markPhase1CompletedForInquiry(inquiryId) {
    const phaseMap = await ensurePhasesExist();
    const phase1Id = phaseMap["1.0"];
    const phase2Id = phaseMap["2.0"];
    if (phase1Id) {
        const phase1Tasks = await prisma_1.prisma.wBSTask.findMany({
            where: { inquiryId, phaseId: phase1Id }
        });
        for (const task of phase1Tasks) {
            await prisma_1.prisma.wBSTask.update({
                where: { id: task.id },
    ...
    if (phase2Id) {
        const firstPhase2Task = await prisma_1.prisma.wBSTask.findFirst({
