...
router.put("/items/:id/status", async (req: Request, res: Response) => {
  try {
...
router.get("/summary/job-wise", async (req: Request, res: Response) => {
  try {
...
function parseSheetRows(sheet: XLSX.WorkSheet): Record<string, any>[] {
  const rawData: any[][] = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: "" });
  if (!rawData || rawData.length === 0) return [];

  let headerRowIndex = -1;
  let headerMap: { index: number; normalizedKey: string }[] = [];
  ...
  for (let i = 0; i < Math.min(15, rawData.length); i++) {
    ...
    const hasHeaderKeyword = normalizedRow.some((k) =>
      ["jobno", "jobid", "itemcode", "itemid", "description", "client", "clientname", "qtyin", "qtyout", "openingstock"].includes(k)
    );
  ...
  if (headerRowIndex === -1) {
    const defaultJson: any[] = XLSX.utils.sheet_to_json(sheet);
